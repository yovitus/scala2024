...
/** The Pigaro object is used as an entry point to the framework. One
 *  starts building a pigaro probabilistic model by using factory
 *  methods from this object. Once the model is initiated (the first
 *  variable is created), one can use the model methods (@see
 *  pigaro.Dist) to add more new variables, hierarchical dependencies,
 *  and observations.
 */
object Pigaro:
