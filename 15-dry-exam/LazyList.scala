...
enum LazyList[+A]:
  ...
  */
  ...
  /* Only works on finite lists */
  def foldLeft[B](z: => B)(f: (B, A) => B): B = // The arrow `=>` in front of the argument type `B` means that the function `f` takes its second argument by name and may choose not to evaluate it.
    this match
...
object LazyList:
  ...
  def unfoldViaFold[A, S](z: S)(f: S => Option[(A, S)]): LazyList[A] =
    f(z).fold(empty[A])((p: (A,S)) => cons(p(0),unfold(p(1))(f)))
